	...
	vals: Vec<(&str, LCell<Value>)>) -> Bindings
{
	...
	for (name, hf) in funs {
		bindings.insert(name.to_string(), lcell(Value::Fn(Func::HFunc(hf), true)));
	}
	for (name, hf) in special {
		bindings.insert(name.to_string(), lcell(Value::Fn(Func::HFunc(hf), false)));
	}
	Bindings {
		bindings: bindings,
		parent: None,
